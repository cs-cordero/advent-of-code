...
from typing import NamedTuple, Optional, List, Generator, Iterable
import re
import sys
...
def combination_generator(absolute_max_num: int,
    ...
    for i in range(max_num, -1, -1):
        ...
        yield from combination_generator(absolute_max_num,
                                         num_of_needed_categories,
                                         next_combo)
...
def solution(part2: bool = False) -> int:
    ...
    for amounts in combination_generator(100, len(ingredients)):
        if part2:
            calories = sum(ingredient.get_calories(amount)
                           for ingredient, amount in zip(ingredients, amounts))
            if calories != 500:
                continue

        score_tuples = [ingredient.get_score(amount)
                        for ingredient, amount in zip(ingredients, amounts)]
